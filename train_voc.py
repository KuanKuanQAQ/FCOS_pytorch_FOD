...
parser.add_argument("--n_gpu", type=str, default='0,1', help="number of gpu threads to use during batch generation")
...
transform = Transforms() # data augment
train_dataset = VOCDataset(root_dir='./foreign_object_dataset',resize_size=[922,1640],
...
#model.load_state_dict(torch.load('./checkpoint/model_TODO.pth'))
...
DECAY_1 = TOTAL_STEPS * DECAY_RATE_1 # first time lr decay
DECAY_2 = TOTAL_STEPS * DECAY_RATE_2 # second time lr decay
assert(DECAY_1 % 1 == 0)
assert(DECAY_2 % 1 == 0)
    ...
    for epoch_step, data in enumerate(train_loader):
        ...
        if GLOBAL_STEPS <= WARMPUP_STEPS:
           ...
           for param in optimizer.param_groups:
               param['lr'] = lr
        if GLOBAL_STEPS == DECAY_1:
           lr = LR_INIT * 0.1
           for param in optimizer.param_groups:
               param['lr'] = lr
        if GLOBAL_STEPS == DECAY_2:
           lr = LR_INIT * 0.01
           for param in optimizer.param_groups:
              param['lr'] = lr
