...
class VOCDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,root_dir,resize_size=[1440,2560],split='trainval',use_difficult=False,is_train = True, augment = None):
        self.root=root_dir
...
if __name__=="__main__":
    ...
    '''
    eval_dataset = VOCDataset(root_dir='../foreign_object_dataset', resize_size=[1440, 2560],
                               split='test', use_difficult=False, is_train=False, augment=None)
    print(len(eval_dataset.CLASSES_NAME))
    '''

    transform = Transforms()
    dataset = VOCDataset(root_dir='../foreign_object_dataset', resize_size=[922.5, 1660],
                         split='trainval', use_difficult=False, is_train=True, augment=None)
    for i in range(1):
        img,boxes,classes=dataset[i]
        img,boxes,classes=(img*255).numpy().astype(np.uint8),boxes.numpy(),classes.numpy()
